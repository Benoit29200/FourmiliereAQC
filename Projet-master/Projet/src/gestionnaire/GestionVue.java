...
public class GestionVue implements Observeur {
	...
	boolean onCanvas(Position position) {
		if (position.getX() < 1 - decalageX / taille)
			return true;
		if (position.getX() > ((canvas.getWidth() / taille) - 1) - decalageX / taille)
			return true;
		if (position.getY() < 1 - decalageY / taille)
			return true;
		if (position.getY() > ((canvas.getHeight() / taille) - 1) - decalageY / taille)
			return true;
		return false;
	}
	...
	public void drawCase(Position position, int element) {
		...
		if (this.taille > 5) {
			...
			gc.fillRect(decalX(position.getX() * taille + 1), decalY(position.getY() * taille + 1), taille - 2,
					taille - 2);
		} else {
			gc.setFill(getColorCase(element));
			gc.fillRect(decalX(position.getX() * taille + 1), decalY(position.getY() * taille + 1), taille - 2,
		...
		if (taille > 5) {
			for (int index = 0; index < nombreFourmis; index++) {
				gc.fillOval(decalX((position.getX() * (taille) + (Math.random() * (taille - 4)) + 2)),
						decalY((position.getY() * (taille) + (Math.random() * (taille - 4)) + 2)), taille / 4,
						taille / 4);
			}
		} else {
			for (int index = 0; index < nombreFourmis; index++) {
				gc.fillOval(decalX(position.getX() * (taille) + taille / 2),
						decalY(position.getY() * (taille) + taille / 2), taille / 4, taille / 4);
			}
		}
	}
	...
	public void drawProie(Position position, boolean vivant) {
		...
		if (taille > 5) {
			gc.fillOval(decalX(position.getX() * (taille) + 2), decalY(position.getY() * (taille) + 2), taille / 2,
					taille / 2);
		} else {
			gc.fillOval(decalX(position.getX() * (taille) + 2), decalY(position.getY() * (taille) + 2), taille / 2,
					taille / 2);
	...
	Color getColorCase(int element) {
		if (element > 0) {
			if (element < SEUIL)
				return Color.color(1 - (double) element / SEUIL, 1, 1 - (double) element / SEUIL);
			else
